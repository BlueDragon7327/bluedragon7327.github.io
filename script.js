...
function initMedia() {
  ...
  // Preload and attempt to play muted to satisfy autoplay policies in some browsers
  try {
    ...
    backgroundMusic.play().then(() => {
      ...
      console.warn('Background music play (muted) failed during init:', err);
    });
  } catch (err) {
    console.warn('Error while preloading background music:', err);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ltcLink = document.getElementById('ltc-link');
  ...
  // LTC copy handler
  if (ltcLink) {
    ltcLink.addEventListener('click', (e) => {
      e.preventDefault();
      const address = ltcLink.dataset.address;
      if (!address) return;
      // Use Clipboard API with fallback
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(address).then(() => {
          showCopyToast('Copied LTC address', ltcLink);
        }).catch((err) => {
          fallbackCopy(address, ltcLink);
        });
      } else {
        fallbackCopy(address, ltcLink);
      }
  ...
  function fallbackCopy(text, anchor) {
    try {
      ...
      showCopyToast('Copied LTC address', anchor);
    } catch (err) {
      alert('Copy failed: ' + (err && err.message ? err.message : err));
    }
  }

  function showCopyToast(msg, anchor) {
    if (!copyToast) return;
    copyToast.textContent = msg;
    // position above anchor if provided
    if (anchor && anchor.getBoundingClientRect) {
      ...
      copyToast.style.transform = 'translate(-50%, -100%)';
    } else {
      // default center bottom
      copyToast.style.left = '50%';
      copyToast.style.top = 'auto';
      copyToast.style.bottom = '120px';
      copyToast.style.transform = 'translateX(-50%)';
    }
  ...
  const name = "𝓑𝓵𝓾𝓮𝓓𝓻𝓸𝓰𝓸𝓫";
  // Typing removed — display static name
  ...
  // Theme buttons removed - no theme switching needed
